...
jobs_data = pd.read_csv('indeed_jobdata_cleaned.csv')
soft_skills = pd.read_csv('soft_skills_frequency.csv')
soft_skills_sorted = soft_skills.sort_values(by='frequency', ascending = True)
hard_skills = pd.read_csv('hard_skills_frequency.csv')
hard_skills_sorted = hard_skills.sort_values(by = 'frequency', ascending = True)
tools = pd.read_csv('tools_frequency.csv')
tools_sorted = tools.sort_values(by = 'frequency', ascending = True)
languages = pd.read_csv('languages_frequency.csv')
...
st.markdown("<h3 style='text-align: center; color: black;'>Top skills and their frequency of appearance in job descriptions</h3>", unsafe_allow_html=True)


# building soft skills bar chart
soft_skills_fig = px.bar(soft_skills_sorted,x='frequency',y='name',color_discrete_sequence =['#152238']*len(soft_skills_sorted))
...
hard_skills_fig = px.bar(hard_skills_sorted,x='frequency',y='name',color_discrete_sequence =['#152238']*len(hard_skills_sorted))
...
tools_fig = px.bar(tools_sorted,x='frequency',y='name',color_discrete_sequence =['#152238']*len(tools_sorted))
...
languages_fig = px.bar(languages_sorted,x='frequency',y='name',color_discrete_sequence =['#152238']*len(hard_skills_sorted))
...
with st.container():
    ...
    with left_column:
        st.markdown("<h4 style='text-align: center; color: black;'>Soft skills</h4>", unsafe_allow_html=True)
        st.plotly_chart(soft_skills_chart)
        st.markdown("<h4 style='text-align: center; color: black;'>Tools</h4>", unsafe_allow_html=True)
        st.plotly_chart(tools_chart)
    with right_column:
        st.markdown("<h4 style='text-align: center; color: black;'>Hard skills</h4>", unsafe_allow_html=True)
        st.plotly_chart(hard_skills_chart)
        st.markdown("<h4 style='text-align: center; color: black;'>Programming languages</h4>", unsafe_allow_html=True)
...
with st.container():
    ...
    with col2:
        st.markdown("<h3 style='text-align: center; color: black;'>Where are the jobs loacated</h3>", unsafe_allow_html=True)
...
with st.container():
    st.write('---')
    st.markdown("<h2 style='text-align: center; color: black;'>Quick search</h2>", unsafe_allow_html=True)
